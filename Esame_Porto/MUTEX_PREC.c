...
struct porto_t
{
    ...
    pthread_cond_t entrata;
    pthread_cond_t uscita;
...
void init_porto(struct porto_t *porto)
{
    ...
    pthread_cond_init(&porto->entrata, &condattr);
    pthread_cond_init(&porto->uscita, &condattr);
...
void entrata_richiestaporto(struct porto_t *porto)
{
    ...
    printf("Barca n.%ld > Devo entrare nel porto.\n In transito %d e il numero di posti occupati è %d\n\n", pthread_self(), 2 - porto->in_transito, porto->in_sosta);

    while (porto->in_sosta == NPOSTI_LIBERI || porto->in_transito == 0 || porto->b_uscita)
    {
        ...
        pthread_cond_wait(&porto->entrata, &porto->mutex);
        porto->b_entrata--;
    }
    porto->in_transito--;
...
void uscita_richiesta(struct porto_t *porto)
{
    ...
    while (porto->in_transito == 0)
    {
        ...
        pthread_cond_wait(&porto->uscita, &porto->mutex);
        porto->b_uscita--;
    }
    porto->in_transito--;
    pthread_mutex_unlock(&porto->mutex);
